...
# The exact JSON provided by the user in the prompt
user_json = """{
"meta": {
...
wf = json.loads(user_json)

def replace_node(old_name, new_node):
    ...
    for key, output in wf["connections"].items():
        for conn_type, target_array in output.items():
            for target_list in target_array:
                for target in target_list:
                    if target["node"] == old_name:
                        target["node"] = new_node["name"]

# 1. WhatsApp Trigger -> WAHA Webhook Trigger
replace_node("WhatsApp Trigger", {
    "id": "waha-trigger",
...
# 2. MongoDB Vector Search -> Postgres Vector Store
replace_node("MongoDB Vector Search", {
...
# 3. MongoDB Vector Store Inserter -> Postgres Vector Store Inserter
replace_node("MongoDB Vector Store Inserter", {
    "id": "postgres-vector-insert",
...
# 4. OpenAI Chat Model -> Gemini Chat Model
replace_node("OpenAI Chat Model", {
    "id": "gemini-chat",
...
# 5. Embeddings OpenAI -> Gemini Embeddings Search
replace_node("Embeddings OpenAI", {
    "id": "gemini-embed-search",
...
# 6. OpenAI Embeddings Generator -> Gemini Embeddings Insert
replace_node("OpenAI Embeddings Generator", {
    "id": "gemini-embed-insert",
...
# 7. Send Response -> WAHA HTTP Request
replace_node("Send Response", {
    "id": "waha-send",
    "name": "Send Response",
    "type": "n8n-nodes-base.httpRequest",
    "typeVersion": 4.2,
...
# 8. Send Unsupported Response -> WAHA HTTP Request Unsupported
replace_node("Send Unsupported Response", {
    "id": "waha-send-unsupported",
    "name": "Send Unsupported Response",
    "type": "n8n-nodes-base.httpRequest",
    "typeVersion": 4.2,
    "parameters": {
...
# Wait, we need to update expressions that explicitly reference "WhatsApp Trigger"
# like: "={{ $('WhatsApp Trigger').item.json.contacts[0].wa_id }}"
# we change them to point to WAHA Webhook Trigger
for n in wf["nodes"]:
    if "parameters" in n:
        for k, v in n["parameters"].items():
            if isinstance(v, str):
                n["parameters"][k] = v.replace("WhatsApp Trigger", "WAHA Webhook Trigger").replace("messages[0]", "body.payload")

filepath = r"C:\Users\AISAR\OneDrive\Desktop\ARCHIVE\KnowledgeBase_Infrastructure\n8n_workflows\jana_waha_exact_match_workflow.json"
with open(filepath, "w", encoding='utf-8') as f:
    json.dump(wf, f, indent=2)

print(f"Saved exact match workflow to {filepath}")
